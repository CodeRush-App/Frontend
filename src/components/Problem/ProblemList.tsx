...
export default function ProblemList({ problems }: { problems: Problem[] }) {
  ...
  function getTagColor(tag: string) {
    return tagColorsMap.get(tag) || tagColors[0];
  }
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      {problems.map((problem) => (
        <ProblemListElement key={problem.id} problem={problem} getTagColor={getTagColor} getDifficultyColor={getDifficultyColor} />
      ))}
    </Box>
  );
}

function ProblemListElement({ problem, getTagColor, getDifficultyColor }: { problem: Problem, getTagColor: (tag: string) => string, getDifficultyColor: (difficulty: string) => string }) {
  return (
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 2, backgroundColor: brandColors.dark, padding: 2, borderRadius: 2 }}>
      <Box>
        ...
        <p>
          ...
          <span>, Success Rate: {getSuccessRate(problem, [problem])}%</span>
        </p>
      </Box>
